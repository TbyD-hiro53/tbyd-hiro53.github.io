/* The Armillary v1 — w1. 海の真ん中の島の山頂、黄昏。地面に少しめり込んだ球電の殻の中で、着色ガラスの四重の環が
 * 方向も速さも不規則に回る。中心の黒いもやから、コードレインのホログラム（脳核・会長室のメトロノーム・Cyberwafer）が
 * 順に浮かび上がる。
 * three.js r128 / 作品スクリプトは単一 IIFE / 同一サイトの資産のみ。
 * 地面・岩・空は Blender（build_g2.py → export_web.py）から。照らされ方は頂点に焼き込み。環・殻・放電・もや・文字は実時間。
 *
 ...
 */
(function(){'use strict';
var T=window.THREE,P='armillary-v1-w1-';
var stage=document.getElementById('stage');
var query=new URLSearchParams(location.search);

/* ---------------------------------------------------------------- 寸法（Blender と同じ。three は y が上） */
var RS=3.2,CZ=2.6,TAU=Math.PI*2;
...
/* 環：軸方向の幅 W、径方向の厚み TH（外面は半径 r の球面、内面は r − TH の球面）。発光の強さは Blender と同じ。
 * 軸でつながず、同心の球帯としてわずかな隙間 GAP で入れ子にする：各環の外面は、一つ外の環の内面のすぐ内側
 * → どの向きに回っても互いに当たらない（作者指示 2026-09-23） */
var RING_W=0.24,RING_TH=0.06,RING_GAP=0.012;
...
RINGS.forEach(function(rd,k){rd.r=2.70-k*(RING_TH+RING_GAP);});
...
var VIEW_DEFS=[
  {key:'wide', name:'全景',pos:bl([15.5,-10.5,2.6]),tgt:bl([0,0,2.9]),lens:28,portrait:1.35,lift:0.0},
  {key:'mid',  name:'中景',pos:bl([6.8,-4.6,3.6]), tgt:bl([0,0,2.55]),lens:32,portrait:1.30,lift:0.0},
  {key:'rings',name:'環',  pos:bl([3.9,-2.3,3.1]), tgt:bl([0,0,2.6]), lens:38,portrait:1.15,lift:0.0},
  {key:'core', name:'核',  pos:bl([2.2,4.1,3.3]),  tgt:bl([0,0,2.6]), lens:40,portrait:1.15,lift:0.0},
  {key:'rim',  name:'縁',  pos:bl([5.2,-1.9,0.55]),tgt:bl([0,0,1.7]), lens:22,portrait:1.10,lift:0.0}
];
...
function makeArcs(){
  ...
  function spawnGround(){
    var a=rnd()*TAU,edge=Math.sqrt(RS*RS-CZ*CZ),p=new T.Vector3(Math.cos(a)*edge,0.01,-Math.sin(a)*edge),pts=[p.clone()];
    var n=8+Math.floor(rnd()*12);
    for(var k=0;k<n;k++){p.add(new T.Vector3(Math.cos(a)*0.09+gauss()*0.06,gauss()*0.02,-Math.sin(a)*0.09+gauss()*0.06));p.y=Math.max(p.y,0)+0.01;pts.push(p.clone());}
    return [pts];
  }
  for(var i=0;i<10;i++)bolts.push({lines:spawn(),life:0.15+rnd()*0.6,age:rnd()*0.5,br:0.6+rnd()*0.6,ground:false,rest:0});
  for(i=0;i<6;i++)bolts.push({lines:spawnGround(),life:0.1+rnd()*0.4,age:rnd()*0.3,br:0.5+rnd()*0.6,ground:true,rest:0});
  var acc=0;
  function update(dt){
...
/* ---------------------------------------------------------------- ジンバル（球帯のガラスの環・軸受けのピン） */
function ringGeometry(r,w,th,seg,nz){
  var prof=[],i,z,ri=r-th;
  for(i=0;i<=nz;i++){z=-w/2+w*i/nz;prof.push([Math.sqrt(r*r-z*z),z,Math.sqrt(r*r-z*z)/r,z/r]);}          /* 外面：法線は球の外向き */
  prof.push([prof[nz][0],w/2,0,1]);                                                                        /* 上の平らな縁 */
  for(i=nz;i>=0;i--){z=-w/2+w*i/nz;prof.push([Math.sqrt(ri*ri-z*z),z,-Math.sqrt(ri*ri-z*z)/ri,-z/ri]);}   /* 内面 */
  prof.push([Math.sqrt(ri*ri-w*w/4),-w/2,0,-1]);                                                            /* 下の平らな縁 */
  var np=prof.length,pos=[],nor=[],idx=[],s,k;
  /* 縁の面は角で法線を分ける（頂点を重ねる） */
  var seq=[];for(k=0;k<np;k++)seq.push(k);
  for(s=0;s<=seg;s++){var a=s/seg*TAU,ca=Math.cos(a),sa=Math.sin(a);
    for(k=0;k<np;k++){var p=prof[k];pos.push(p[0]*ca,p[1],-p[0]*sa);nor.push(p[2]*ca,p[3],-p[2]*sa);}}
  for(s=0;s<seg;s++)for(k=0;k<np-1;k++){var A=s*np+k,B=(s+1)*np+k;idx.push(A,B,A+1,B,B+1,A+1);}
...
function makeRings(){
  ...
  RINGS.forEach(function(rd,k){
    var g=ringGeometry(rd.r,RING_W,RING_TH,256,10);
    ...
    var GL_T=['uniform vec3 uCol;uniform float uAbs;',
      'vec3 trans(float c){float path='+(RING_TH*0.5).toFixed(3)+'/max(c,0.12);return exp(-uAbs*path*(vec3(1.0)-uCol))*(1.0-(0.04+0.96*pow(1.0-c,5.0)));}'].join('\n');
    ...
    var mAdd=new T.ShaderMaterial({transparent:true,depthWrite:false,side:T.DoubleSide,blending:T.AdditiveBlending,
      ...
      fragmentShader:[GL_T,'uniform float uGlow;uniform sampler2D tSky,tBg;uniform vec2 uRes;varying vec3 vW;varying vec3 vN;',GL_DECODE,GL_ENV,
        ...
        /* 屈折：厚み 6 cm の薄い帯なので像のずれはわずか。手前の面だけ（裏の面や重なりで二重に引かれて暗い残像になるのを防ぐ） */
...
function build(geoDoc,geoBin,holoDoc,holoBin,tex){
  SKY_TEX=tex.sky;PATCH_ALB=tex.palb;PATCH_NRM=tex.pnrm;PATCH_SIZE=(geoDoc.patch&&geoDoc.patch.size)||32;
  scene=new T.Scene();
