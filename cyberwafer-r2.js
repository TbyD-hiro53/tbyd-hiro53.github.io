...
(function(){'use strict';
var T=window.THREE,P='cyberwafer-r2-',el=function(id){return document.getElementById(id);};
...
function fail(e){console.error(e);el('loading').className='failed';el('loading').innerHTML='<p>立体の読み込みができませんでした。</p><a href="cyberwafer-r2-original-hero.jpg">Blenderで描いた写真を開く</a><p><a href="cyberwafer-r2-preview.html">もう一度読み込む</a></p>';el('photograph').hidden=false;}
try{
...
Promise.all([response(query.get('qa')==='missing'?P+'missing-test.json':P+'scene.json','json'),response(P+'space.f16'),tex(P+'gradHo.png',true),tex(P+'gradEm.png',true),tex(P+'waferD.png',true)]).then(function(data){
 ...
 if(e==='glass'){m.transmission=1;m.ior=1.46;m.thickness=n==='T'?1.05:n==='DiscTop'?.07:.038;m.transparent=false;m.opacity=1;}
...
el('photo').onclick=function(){photo=!photo;if(photo)setPhoto(edition);el('photograph').hidden=!photo;document.body.classList.toggle('photographic',photo);el('photo').textContent=photo?'立体へ':'写真';el('photo').setAttribute('aria-pressed',String(photo));};
