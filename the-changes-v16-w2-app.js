/* 車窓 / THE CARRIAGE WINDOW（旧題 乗換 / THE CHANGES）v16 w2 — 夜の車窓（原作『書 / THE WRITTEN』篇01・03・05〜07）。
...
/* 窓のガラスの中では駅の灯の層を使わない（層を掛け算で 1 − T だけ弱めると 10〜15 % 残り、層に刷り込まれた樹の形が夜の露出でうっすら出た。作者の指摘 2026-09-26） */
' c+=sl*chroma*vec3(1.,.95,.88)*(1.-smoothstep(.02,.15,Tn));',
...
async function load(progress){
 ...
 const file=k=>k==='lut'?(meta.lut_file||P+'lut.png'):k==='ids'?meta.ids_file:L[k].file;
