/* The Armillary v1 を、公開中の作品と同じ共有UI構成へ接続するアダプタ。
 * w2（2026-09-24、作者指示）：題の下の署名の下に「歴年のSF作品への熱烈なオマージュ」の一行を足す（英語表示では英文）。
 *   署名は共有 composer の固定の文字なので触らず、この作品の題の欄にだけ行を足す。
 * sanctum-v6-w3-ui.js を元に、名前と視点の割り当てだけを差し替えた（gimbal-claude/web-src/make_page.py）。
 * w3：毎フレームの書き戻しは値が変わったときだけ（make_w3.py）。h53-chrome が釦の字を英語へ置き換えるので、
 *     同じ値を毎フレーム書くと翻訳と書き合いになり、DOM の変更が毎フレーム 5 件ずつ起きていた。
...
(function(root){
  ...
  var VIEW_BUTTONS=[
    {id:'sw',    key:'wide',   label:'全景'},
    {id:'reset', key:'mid',    label:'中景'},
    {id:'spin',  key:'rings',  label:'環'},
    {id:'tour',  key:'core',   label:'核'},
    {id:'era',   key:'rim',    label:'縁'}
  ];
  var MOTION_ID='dp';
  ...
  function publishCopy(){
    var copy=root.ArmillaryUICopy;
    if(!copy)throw Error('Armillary UI copy is missing');
  ...
  var lastView=-1,lastPlaying=null;
  function syncLegacy(){
    if(!api)return;
    var now=api.getViewIndex(),i,b,playing=!!api.getPlaying();
    if(now===lastView&&playing===lastPlaying)return;
    ...
    if(m&&playing!==lastPlaying){m.textContent=playing?'motion':'paused';m.classList.toggle('on',playing);}
    var label=byId('current-view'),list=views();
    if(label&&list[now]&&now!==lastView)label.textContent=list[now].name;
    lastView=now;lastPlaying=playing;
  }
  ...
  function wireLegacy(){
    ...
    if(missing.length)throw Error('Armillary controls missing: '+missing.join(','));
    /* 拡大縮小は作品面で直接（ホイール・二本指）。釦もスライダも置かない。
       行き過ぎは作品側の constrain() が床・天井・壁・基壇・ガラスの環で止める。 */
    syncLegacy();
  }
  ...
  function getOccupancy(){
    var probe=byId('armillarySafeArea');
  ...
  /* オマージュの表記：題の欄（composer-title：題と署名）の下に一行。言語は h53-chrome と同じ設定（localStorage h53lang）に従う */
  var HOMAGE={ja:'歴年のSF作品への熱烈なオマージュ',en:'An ardent homage to the science fiction of years past'};
  function addHomage(){
    var t=document.querySelector('.composer-title');
    if(!t||t.querySelector('.armillary-homage'))return;
    var lang='ja';try{if(localStorage.getItem('h53lang')==='en')lang='en';}catch(e){}
    var s=document.createElement('small');s.className='armillary-homage';s.lang=lang;s.textContent=HOMAGE[lang];
    s.style.cssText='display:block;margin-top:3px;opacity:.72;letter-spacing:.02em';
    t.appendChild(s);notifyLayout();
  }
  ...
  function loadComposer(){
    ...
    node.addEventListener('load',function(){addHomage();watchDock();notifyLayout();requestFrame();setTimeout(function(){addHomage();watchDock();notifyLayout();syncLegacy();},400);});
    node.addEventListener('error',function(){document.documentElement.dataset.uiError='共有UIの composer を読み込めませんでした。';});
    document.body.appendChild(node);
  }

  function mount(options){
    if(mounted)return root.ArmillaryUI;
    ['views','getViewIndex','setView','reset','zoom','getPlaying','setPlaying','requestFrame'].forEach(function(k){
      if(!options||typeof options[k]!=='function')throw Error('Armillary UI API missing '+k);
    });
    if(!options.canvas)throw Error('Armillary UI requires the final artwork canvas');
    api=options;
    publishCopy();
    wireLegacy();
    mounted=true;
  ...
  root.ArmillaryUI={
    ...
    report:function(){
      var c=composer();
      return {adapter:'armillary-v1-w2',mounted:mounted,views:mounted?views().length:0,
