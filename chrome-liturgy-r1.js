...
(function(){'use strict';
var T=window.THREE,P='chrome-liturgy-r1-',el=function(id){return document.getElementById(id);};
...
function fail(e){console.error(e);el('loading').className='failed';el('loading').innerHTML='<p>立体の読み込みができませんでした。</p><a href="chrome-liturgy-r1-hero.jpg">Blenderで描いた写真を開く</a><p><a href="chrome-liturgy-r1-preview.html">もう一度読み込む</a></p>';el('photograph').hidden=false;}
try{
...
Promise.all([response(query.get('qa')==='missing'?P+'missing-test.json':P+'scene.json','json'),response(P+'space.f16')]).then(function(data){
 var spec=data[0];envTexture=new T.DataTexture(new Uint16Array(data[1]),1024,512,T.RGBAFormat,T.HalfFloatType);envTexture.encoding=T.LinearEncoding;envTexture.mapping=T.EquirectangularReflectionMapping;envTexture.needsUpdate=true;
 var gen=new T.PMREMGenerator(renderer);pmremTexture=gen.fromEquirectangular(envTexture).texture;scene.environment=pmremTexture;gen.dispose();envTexture.dispose();
 var materials={};Object.keys(spec.materials).forEach(function(name){var d=spec.materials[name],m=new T.MeshPhysicalMaterial({color:new T.Color().fromArray(d.color),metalness:d.metalness,roughness:d.roughness,clearcoat:d.clearcoat,clearcoatRoughness:d.clearcoatRoughness,emissive:new T.Color().fromArray(d.emissive),emissiveIntensity:d.emissiveIntensity,envMapIntensity:1});
  if(name.indexOf('R1 T /')===0){surface(m,'t');m.envMapIntensity=1.75;}if(name.indexOf('R1 disc /')===0){surface(m,'disc');m.envMapIntensity=1.35;}if(name.indexOf('space /')>=0){m.side=T.DoubleSide;m.envMapIntensity=.38;}materials[name]=m;
 });var done=0;
 ...
 ready=true;resize();if(review){yaw=Math.atan2(11,19);pitch=Math.atan2(4.9,Math.hypot(11,19));dist=Math.hypot(11,19,4.9);camera.fov=2*Math.atan(36/120)*180/Math.PI;camera.updateProjectionMatrix();}updateModel(0);updateCamera();updateCubes();updateCubes();el('loading').hidden=true;requestAnimationFrame(frame);
}).catch(fail);
...
function resize(){var w=innerWidth,h=innerHeight;mobile=w<700;renderer.setPixelRatio(Math.min(devicePixelRatio,high?1.5:mobile?1.2:1.5));renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();
 var px=renderer.getPixelRatio();fxTarget.setSize(Math.round(w*px),Math.round(h*px));fxMaterial.uniforms.resolution.value.set(1/(w*px),1/(h*px));var head=document.querySelector('header').getBoundingClientRect(),title=el('title').getBoundingClientRect(),controls=el('controls').getBoundingClientRect();var top=Math.max(head.bottom,title.bottom)+30,bottom=controls.top-24,usable=Math.max(.3,(bottom-top)/h);
...
function immersive(){document.body.classList.toggle('immersive');el('restore').hidden=!document.body.classList.contains('immersive');el('fullscreen').textContent=document.body.classList.contains('immersive')?'通常表示へ':'全画面';}el('restore').onclick=immersive;el('fullscreen').onclick=function(){var d=document.documentElement;if(document.fullscreenElement){document.exitFullscreen();}else if(d.requestFullscreen&&query.get('qa')!=='fallback'){var guard=setTimeout(function(){if(!document.fullscreenElement&&!document.body.classList.contains('immersive'))immersive();},1200);d.requestFullscreen().catch(function(){clearTimeout(guard);if(!document.body.classList.contains('immersive'))immersive();});}else{immersive();}};
