...
function start(){
 ...
 function menu(){view='menu';heading.textContent='メニュー';content.replaceChildren();mirrors=[];zoomMirror=null;
  ...
  action('UIを隠す',function(){closePanel(function(){hidden=true;top.hidden=true;name.hidden=true;show.hidden=false;show.focus({preventScroll:true});});});
 }
 ...
 function message(label,text){view='about';mirrors=[];stat=null;heading.textContent=label;content.replaceChildren();content.append(el('p',{},text));action('メニューへ戻る',menu);}
 ...
 close.addEventListener('click',function(e){e.stopPropagation();closePanel();});show.addEventListener('click',function(){hidden=false;top.hidden=false;name.hidden=false;show.hidden=true;toggle.focus({preventScroll:true});});
 ...
 current={host:host,canvas:canvas,slug:slug,report:function(){return {slug:slug,view:view,hidden:hidden,legacy:mirrors.map(function(x){return {id:x.source.id,text:x.button.textContent,disabled:x.button.disabled,on:x.button.getAttribute('aria-pressed')};}),host:host.report()};},afterRender:function(){prepareSource();if(!panel.hidden)sync();host.afterRender(performance.now());}};
