...
function closeInfo(){$('info').hidden=true;if(returnFocus&&returnFocus.isConnected&&!returnFocus.closest('[hidden]'))returnFocus.focus();else $('menuToggle').focus();schedule();}
...
$('settings').onclick=()=>{openInfo('画質・動き');const label=document.createElement('label'),check=document.createElement('input');check.type='checkbox';check.checked=reduced;check.onchange=()=>{reduced=check.checked;document.body.classList.toggle('reduce-ui-motion',reduced);schedule();};label.append(check,document.createTextNode(' 風と水面の細かな動きを減らす'));$('infoBody').append(label);const ql=document.createElement('label');ql.textContent='画質';const sel=document.createElement('select');for(const [value,text] of [['auto','自動'],['high','高'],['low','軽量']]){const op=document.createElement('option');op.value=value;op.textContent=text;sel.append(op);}sel.value=world.quality;sel.onchange=()=>{world.quality=sel.value;world.scale=1;resize();schedule();};ql.append(sel);$('infoBody').append(ql);};
function syncUI(){const t=clock.sunsetSeconds,state=C.duskState(t);$('sunset').disabled=!ready||clock.sunsetStarted;$('sunset').setAttribute('aria-pressed',String(clock.sunsetStarted));$('phase').textContent=!clock.running?'一時停止':!clock.sunsetStarted?'夕景':state.suns?'日没':t<225?'残照':'夕闇';$('pause').textContent=clock.running?'一時停止':'再開';$('pause').setAttribute('aria-pressed',String(!clock.running));}
