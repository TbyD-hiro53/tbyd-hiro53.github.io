...
(function(){'use strict';
var T=window.THREE,P='lacto-cortex-v10-c1-',stage=document.getElementById('stage');
...
function setView(i,automatic){
  ...
  if(v.kind==='orbit'){
    lensFov=null;camera.near=.02;camera.far=400;target.set(v.target[0],v.target[1],v.target[2]);theta=v.theta;phi=v.phi;radius=v.radius;updateCamera();
  }else{
...
function cLayoutChanged(){if(ready&&r3HeroFitActive)setView(r3AutoDefault?r3DefaultView():viewIndex,true);needsRender=true;}
...
function cFeedInit(){
  ...
  if(tube&&!tube.userData.cPulse){
    ...
    m.needsUpdate=true;tube.userData.cPulse=true;cFeedReady=true;
  }
  scene.traverse(function(o){
...
function init(){
  ...
  Promise.all([response(P+'geo.json','json'),parts(P+'geo',4),
