/* CHROME LITURGY — approved r3 artwork with opt-in post-display Liquid Glass UI hook. © h!ro53. */
(function(){'use strict';
...
function frame(ms){var frameStart=performance.now();requestAnimationFrame(frame);if(!last)last=ms;var rawFrameMs=ms-last;var dt=Math.min(.1,rawFrameMs/1000);if(mobile&&ms-last<31)return;last=ms;if(document.hidden||!ready)return;if(photo){if(window.ChromeLiturgyLiquid)window.ChromeLiturgyLiquid.afterPhoto(ms);return;}
...
if(window.ChromeLiturgyLiquid)window.ChromeLiturgyLiquid.runtime=diagnostics;
