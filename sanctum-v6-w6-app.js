/* Sanctum v6 — w6（w1→w2 make_w2.py … w4→w5 make_w5.py、w5→w6 make_w6.py：素線の太さ・揺らぎ・速さ、写実の作り込み）. 光学ガラスの環に三条の光を封じ、円形の室の中央に据える。
 ...
 */
(function(){'use strict';
var T=window.THREE,P='sanctum-v6-w1-';   /* 資産は w1 のまま（w2/w3 で替わったのは作品スクリプトと UI アダプタだけ） */
...
function makeRoomMaterial(tex){
  return new T.ShaderMaterial({
    ...
    uniforms:{tLM:{value:tex.lm},tR0:{value:null},tR1:{value:null},mR0:{value:texMat0},mR1:{value:texMat1},
      tEnv:{value:tex.envGlass},clipY:U.clipY,uLM:{value:1.0},uRefl:{value:1.0},uDebug:{value:0},uCheap:{value:0},uRingCol:{value:ringColor()}},
    vertexShader:[
    ...
    fragmentShader:[
      'uniform sampler2D tLM,tR0,tR1,tEnv;uniform mat4 mR0,mR1;uniform float clipY,uLM,uRefl;uniform int uDebug;uniform int uCheap;uniform vec3 uRingCol;',
      ...
      ' }else if(role==4){col+=(envAt(tEnv,reflect(-V,N),1.0)*0.6+ringSheen(reflect(-V,N),60.0))*F;}',
      /* 壁と付柱（本磨きでない石）：粗い艶。光条の色が斜めから見た面にうっすら乗る */
      ' if(role==1)col+=ringSheen(reflect(-V,N),7.0)*(0.035+0.4*pow(1.0-cosv,5.0))*0.9;',
...
function makeFrontGlassMaterial(tex){
  ...
  return new T.ShaderMaterial({
    ...
    fragmentShader:[
      ...
      ' vec2 o1=nv.xy*uRefr*g*asp*(1.0+0.9*wall);vec2 o2=nv.xy*uRefr*3.2*g*g*asp;',
      ...
      ' vec3 col=refr*(1.0-F)*tint+env*F+inner*F*0.55+rimLine*(env*0.5+inner*0.22+refr*0.10);',
      ' gl_FragColor=vec4(col,1.0);}'
    ].join('\n')
  });
}
