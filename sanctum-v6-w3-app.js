/* Sanctum v6 — w3（w1→w2 は web-src/make_w2.py、w2→w3 は make_w3.py）. 光学ガラスの環に三条の光を封じ、円形の室の中央に据える。
...
var LANE={gap:0.048,core:{r:0.013,s:13.0},glow:{r:0.030,s:13.0},haze:{r:0.085,s:1.4},
  ...
  /* 揺らぎ：三本は横一列に揃わず、束の中でわずかに不規則に揺らぐ。
   * 横（並びの向き）±約 17 mm・縦（束の外向き）±約 17 mm。周方向の波数は整数なので一周で途切れない */
  wob:{side:0.013,up:0.014}};
...
function diagTick(now,gap,cpu,ui,gpu){
  ...
  DIAG.t0=now;DIAG.frames=0;DIAG.gaps=[];DIAG.cpu=[];DIAG.ui=[];DIAG.gpuMs=[];
}
...
function strandMaterial(s,ph,radius,mode,strength,lane,li,lp){
  return new T.ShaderMaterial({
    ...
    vertexShader:[
      ...
      ' vU=aU;vN=n;vec3 P=C+off+n*uR;vW=P;',
...
function frame(now){
  ...
  if(needsRender&&ready){
    ...
    needsRender=false;
    var t2=performance.now();if(window.SanctumUI&&!diagOff('ui'))window.SanctumUI.afterRender(now);
    diagTick(now,gap,t1-t0,performance.now()-t2,gms);
  }
