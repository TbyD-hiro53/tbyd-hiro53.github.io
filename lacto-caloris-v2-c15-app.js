...
var view='turntable',orbit={az:0,el:0,d:1},orbitGoal=null,orbitFrom=null,orbitT=1;
...
function material(name,role){
 ...
 if(LAMP_MATS[name]){
  m.onBeforeCompile=function(shader){
   shader.uniforms.uLampTime=lampTime;
   shader.vertexShader='attribute float lampId;\nvarying float vLampId;\n'+shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\n vLampId=lampId;');
   shader.fragmentShader='uniform float uLampTime;\nvarying float vLampId;\n'+LAMP_GLSL+'\n'+shader.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\n totalEmissiveRadiance*=h53Flicker(vLampId,uLampTime);');
  };
  m.customProgramCacheKey=function(){return 'c15-lampflicker-2';};
 }
 medium(m,role);materials[key]=m;return m;
}

// ---- 各部の表示灯を 1 個ずつ不規則に明滅させる（室内灯・輪の照明・培養槽の導光は対象外）----
var LAMP_MATS={'v2 node accent':1,'v2 coupler lock ring accent':1,'v2 hub node accent (in vat)':1,
 'v2 s4 tray_lamp lens #e8eef2':1,'v2 s4 skin_lamp lens #e8eef2':1,'v2 s4 rim_lamp lens #e8eef2':1,
 'v2 s4 breaker status #ff05a8':1,'v2 CDU ring 00ddc8':1,'v2 CDU ring f2dbe9':1,'v2 CDU ring ff05a8':1,
 'c4 status ff05a8':1,'c4 turq line 00ddc8':1};
var lampTime={value:0};
var LAMP_GLSL=[
 // GPU の float 精度で sin が崩れないよう、ハッシュの引数を小さく保つ（id 最大 3455）
 'float h53h(float n){return fract(sin(n)*43758.5453123);}',
 'float h53Flicker(float id,float t){',
 ' float k=id*0.0173;',
 ' float a=h53h(k+0.31),b=h53h(k*1.7+2.13);',
 ' float sp=.45+2.4*b;',
 ' float base=.80+.20*sin(t*sp+a*6.2831853);',
 ' base*=.90+.10*(sin(t*sp*3.7+b*6.2831853)*.5+.5);',
 ' float seg=floor(mod(t,180.0)*2.3+a*10.0);',
 ' float g=h53h(seg*0.113+k*2.7+0.7);',
 ' float dip=g>.93?(.25+.45*h53h(seg*0.317+k+3.1)):1.0;',
 ' return clamp(base*dip,.05,1.30);}'].join('\n');
// 稜線でつながった頂点を 1 個のランプとみなし、頂点ごとに個体番号を振る（書き出しは変えない）
function lampIds(g){
  var idx=g.index.array,pos=g.attributes.position.array,n=g.attributes.position.count,par=new Int32Array(n),i;
  for(i=0;i<n;i++)par[i]=i;
  function find(x){while(par[x]!==x){par[x]=par[par[x]];x=par[x];}return x;}
  function uni(x,y){x=find(x);y=find(y);if(x!==y)par[y]=x;}
  // 書き出しは法線・UV ごとに頂点を割るので、まず位置で溶接してから島を取る
  var weld={},key,j;
  for(i=0;i<n;i++){
   j=i*3;key=(pos[j]*1e4|0)+'_'+(pos[j+1]*1e4|0)+'_'+(pos[j+2]*1e4|0);
   if(weld[key]===undefined)weld[key]=i;else uni(weld[key],i);
  }
  for(i=0;i<idx.length;i+=3){uni(idx[i],idx[i+1]);uni(idx[i+1],idx[i+2]);}
  var map={},next=0,out=new Float32Array(n),r;
  for(i=0;i<n;i++){r=find(i);if(map[r]===undefined)map[r]=next++;out[i]=map[r];}
  g.setAttribute('lampId',new THREE.BufferAttribute(out,1));
  return next;
}
function build(buf,bake){
 ...
 data.objects.forEach(function(r){
  ...
  if(r.materials.some(function(nm){return LAMP_MATS[nm];}))lampIds(g);
...
var DMIN=.28,DFAR=120;
// カメラの距離は「壁・床・機体に当たらない範囲」だけで決める。近すぎれば外へ、遠すぎれば内へ寄せる。
function safeDistance(dir,d){
 var p=new THREE.Vector3();
 function ok(t){p.copy(target).addScaledVector(dir,t);return safeCameraPoint(p);}
 if(ok(d))return d;
 // 当たっているときは、DMIN〜DFAR を対数で刻んで走査し、要求値にいちばん近い安全な距離を採る
 var lo=Math.log(DMIN),hi=Math.log(DFAR),n=256,ld=Math.log(d),best=-1,bd=1e9,i,t,v;
 for(i=0;i<=n;i++){t=Math.exp(lo+(hi-lo)*i/n);if(ok(t)){v=Math.abs(Math.log(t)-ld);if(v<bd){bd=v;best=t;}}}
 return best>0?best:d;
}
...
function applyOrbit(){var v=VIEWS[view],portrait=innerWidth<innerHeight;target.set(v.t[0],v.t[1]+(portrait?v.pty:0),v.t[2]);var el=Math.max(-1.2,Math.min(1.45,orbit.el)),scale=portrait?v.pd:1,d=Math.max(DMIN,orbit.d)*scale;
 var dir=new THREE.Vector3(Math.sin(orbit.az)*Math.cos(el),Math.sin(el),Math.cos(orbit.az)*Math.cos(el));
 var dd=safeDistance(dir,d);if(dd!==d)orbit.d=dd/scale;
 var p=target.clone().addScaledVector(dir,dd);
