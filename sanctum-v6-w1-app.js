/* Sanctum v6 — w1. 光学ガラスの環に三条の光を封じ、円形の室の中央に据える。
 ...
 */
(function(){'use strict';
var T=window.THREE,P='sanctum-v6-w1-';
...
/* 光条の強さ（Blender s4/s5 と同じ値）：芯 11（面の発光）/ 気体 11・2.2（体積の発光、単位長さあたり） */
var CORE={r:0.020,s:11.0},GAS=[{r:0.045,s:11.0},{r:0.075,s:2.2}];
...
/* 描画解像度の自動調整：再生中のフレーム間隔の中央値が 40 ms（25 fps 未満）を超える状態が続いたら一段ずつ下げる（上げ戻しはしない）。
 * iPhone の低電力モードは描画の重さと無関係に 30 fps（33 ms）へ抑えるので、それでは下げない */
var quality={scale:1,gaps:[],cool:0};
...
var GL_NOISE=[
 ...
 'float fbm(vec3 p,float g){float a=0.5,s=0.0,n=0.0;for(int i=0;i<5;i++){s+=a*vnoise(p);n+=a;p*=2.02;a*=g;}return s/n;}',
...
function makeRoomMaterial(tex){
  return new T.ShaderMaterial({
    ...
    uniforms:{tLM:{value:tex.lm},tR0:{value:null},tR1:{value:null},mR0:{value:texMat0},mR1:{value:texMat1},
      tEnv:{value:tex.envGlass},clipY:U.clipY,uLM:{value:1.0},uRefl:{value:1.0},uDebug:{value:0}},
    vertexShader:[
    ...
    fragmentShader:[
      'uniform sampler2D tLM,tR0,tR1,tEnv;uniform mat4 mR0,mR1;uniform float clipY,uLM,uRefl;uniform int uDebug;',
      ...
      '  alb=mix(alb,vec3(0.012),j);rough=mix(0.06,0.20,fbm(b*1.6,0.5));rough=max(rough,0.7*j);refl=1.0-j*0.8;',
...
function ringColor(){var c=[0,0,0];STRANDS.forEach(function(s){for(var i=0;i<3;i++)c[i]+=(s.glow[i]*(GAS[0].s*GAS[0].r*2*0.6+GAS[1].s*GAS[1].r*2*0.6)+s.core[i]*CORE.s*CORE.r*3.0);});
...
function makePartsMaterial(tex){
  return new T.ShaderMaterial({
    ...
    fragmentShader:[
      ...
      ' for(int k=0;k<64;k++){float a=float(k)/64.0*6.2831853;vec3 L=vec3(cos(a)*'+TOR.R.toFixed(3)+','+TOR.y.toFixed(3)+',sin(a)*'+TOR.R.toFixed(3)+')-vW;',
      '  float d2=max(dot(L,L),0.04);L*=inversesqrt(d2);',
      '  spec+=uRingCol*pow(max(dot(R,L),0.0),n)*(n+2.0)/(6.2831853*d2);diff+=uRingCol*max(dot(N,L),0.0)/d2;}',
      ' spec*=0.069/64.0;diff*=0.069/64.0/3.1415927;}',
...
function strandMaterial(s,ph,radius,mode,strength){
  return new T.ShaderMaterial({
    ...
    uniforms:{uTime:U.time,uPh:{value:ph},uRs:{value:TOR.rs},uSpin:{value:0},uR:{value:radius},uS:{value:strength},
      uGlow:{value:new T.Vector3().fromArray(s.glow)},uCore:{value:new T.Vector3().fromArray(s.core)},uOff:{value:ph/TAU},clipY:U.clipY},
    vertexShader:[
      'attribute float aU;attribute float aA;uniform float uR;uniform float uOff;',GL_STRAND,
      'varying vec3 vW;varying vec3 vN;varying float vU;varying float vBreath;',
      ...
      ' float s1=0.5+0.5*sin((aU-uTime/3.0+uOff)*37.699112);',
      ' vBreath=0.90+0.20*s1;vU=aU;vN=n;vec3 P=C+n*uR*vBreath;vW=P;',
      ' gl_Position=projectionMatrix*viewMatrix*vec4(P,1.0);}'
    ].join('\n'),
    fragmentShader:[
      'uniform float uTime;uniform float uR;uniform float uS;uniform vec3 uGlow;uniform vec3 uCore;uniform float uOff;uniform float clipY;',
      'varying vec3 vW;varying vec3 vN;varying float vU;varying float vBreath;',
      'void main(){if(vW.y<clipY)discard;vec3 V=normalize(cameraPosition-vW);float f=clamp(abs(dot(normalize(vN),V)),0.0,1.0);',
      /* 流れる縞：三つの周波数の重ね（旧 v5 と同じ比）。明暗の幅は控えめに */
      ' float fp=vU-uTime/3.0+uOff;',
      ' float st=0.45*(0.5+0.5*sin(fp*37.699112))+0.35*(0.5+0.5*sin((vU-uTime/3.0*1.15)*81.681409+2.1))+0.20*(0.5+0.5*sin((vU-uTime/3.0*0.85)*144.51326+4.4));',
      ' float I=0.78+0.44*st;',
      mode==='core'
        ?' vec3 col=mix(uGlow,uCore,pow(f,2.5))*uS*pow(f,1.6)*pow(f,0.7)*I;'
        :' vec3 col=uGlow*uS*(2.0*uR*vBreath*f)*I;',
      ' gl_FragColor=vec4(col,1.0);}'
    ].join('\n')
  });
}
...
function build(doc,bin,tex){
  ...
  var geoCore=strandGeometry(720,12),geoGas=strandGeometry(720,14);
  STRANDS.forEach(function(s,i){
    var ph=i*TAU/3;
    GAS.slice().reverse().forEach(function(g){var m=new T.Mesh(geoGas,strandMaterial(s,ph,g.r,'gas',g.s));m.renderOrder=10;m.frustumCulled=false;plasma.add(m);strandObjs.push(m);});
    var c=new T.Mesh(geoCore,strandMaterial(s,ph,CORE.r,'core',CORE.s));c.renderOrder=11;c.frustumCulled=false;plasma.add(c);strandObjs.push(c);
...
function resize(){
  ...
  var cap=query.has('lite')?1.4e6:2.6e6;if(w*h*dpr*dpr>cap)dpr=Math.sqrt(cap/(w*h));
  ...
  if(!rtA){rtA=makeRT(W,H,{depth:true,msaa:true});rtB=makeRT(W,H,{depth:true,msaa:true});
    rtR0=makeRT(W>>1,H>>1,{depth:true});rtR1=makeRT(W>>1,H>>1,{depth:true});
    for(var i=0;i<7;i++)bloomRT.push(makeRT(1,1,{}));}
  rtA.setSize(W,H);rtB.setSize(W,H);rtR0.setSize(Math.max(1,W>>1),Math.max(1,H>>1));rtR1.setSize(Math.max(1,W>>1),Math.max(1,H>>1));
...
function mirrorFrom(h,texMat,rt){
  ...
  U.clipY.value=h+0.002;backGlass.visible=false;
  ...
  U.clipY.value=-1e3;backGlass.visible=true;
}

/* ---------------------------------------------------------------- 一枚 */
function render(){
  camera.near=0.02;camera.far=40;camera.updateProjectionMatrix();
  mirrorFrom(0.0,texMat0,rtR0);
  mirrorFrom(ROOM.daisH,texMat1,rtR1);
  roomMat.uniforms.tR0.value=rtR0.texture;roomMat.uniforms.tR1.value=rtR1.texture;
  renderer.setRenderTarget(rtA);renderer.setClearColor(0x000000,1);renderer.clear();renderer.render(sceneA,camera);
  copyMat.uniforms.t.value=rtA.texture;frontGlassMat.uniforms.tScene.value=rtA.texture;
  renderer.setRenderTarget(rtB);renderer.clear();renderer.render(sceneB,camera);
  /* にじみ */
...
function frame(now){
  ...
  if(playing&&ready&&gap>0&&gap<250){quality.gaps.push(gap);if(quality.gaps.length>90)quality.gaps.shift();
    if(quality.gaps.length===90&&now>quality.cool&&quality.scale>0.55){var g=quality.gaps.slice().sort(function(a,b){return a-b;})[45];
      if(g>40){quality.scale=Math.max(0.55,quality.scale*0.8);quality.gaps=[];quality.cool=now+2500;resize();}}}
