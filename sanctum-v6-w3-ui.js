/* Sanctum v6 を、公開中 lacto-caloris v2 / lacto-cortex v10 と同じ共有UI構成へ接続するアダプタ。
 * lacto-cortex-v10-c3-ui.js を元に、名前と視点の割り当てだけを差し替えた（sanctum-claude/web-src/make_ui.py）。
...
(function(root){
  ...
  var VIEW_BUTTONS=[
    {id:'sw',    key:'room',   label:'全景'},
    {id:'reset', key:'hall',   label:'斜め'},
    {id:'spin',  key:'above',  label:'真上'},
    {id:'tour',  key:'ring',   label:'環'},
    {id:'era',   key:'saddle', label:'受け'}
  ];
  var MOTION_ID='dp';
  ...
  function publishCopy(){
    var copy=root.SanctumUICopy;
    if(!copy)throw Error('Sanctum UI copy is missing');
  ...
  function wireLegacy(){
    ...
    if(missing.length)throw Error('Sanctum controls missing: '+missing.join(','));
  ...
  function getOccupancy(){
    var probe=byId('sanctumSafeArea');
  ...
  function mount(options){
    if(mounted)return root.SanctumUI;
    ['views','getViewIndex','setView','reset','zoom','getPlaying','setPlaying','requestFrame'].forEach(function(k){
      if(!options||typeof options[k]!=='function')throw Error('Sanctum UI API missing '+k);
    });
    if(!options.canvas)throw Error('Sanctum UI requires the final artwork canvas');
    api=options;
    publishCopy();
    wireLegacy();
    mounted=true;
  ...
  root.SanctumUI={
    ...
    report:function(){
      var c=composer();
      return {adapter:'sanctum-v6-w3',mounted:mounted,views:mounted?views().length:0,
