...
(function(){'use strict';
var T=window.THREE,P='lacto-cortex-v10-c1-',PG='lacto-cortex-v10-c2-',PL='lacto-cortex-v10-c3-'   /* P 据え置き ／ PG 立体 ／ PL この版で替わった資産 */,stage=document.getElementById('stage');
...
var R3_BOUNDS={floor:-2.18,ceiling:21.8,radius:18.1,machineRadius:.8,machineTop:2.15,panelX:0,panelZ:-5.0,panelRadius:1.06,panelTop:-1.22};
...
function constrain(){
  ...
  var dxp=p.x-b.panelX,dzp=p.z-b.panelZ,dhp=Math.hypot(dxp,dzp);
  if(p.y<b.panelTop&&dhp<b.panelRadius){
    if(dhp>1e-8){p.x=b.panelX+dxp*b.panelRadius/dhp;p.z=b.panelZ+dzp*b.panelRadius/dhp;}
    else{p.x=b.panelX+b.panelRadius;p.z=b.panelZ;}
  }
}
...
function init(){
  ...
  Promise.all([response(PG+'geo.json','json'),parts(PG+'geo',4),
    partsTexture(PL+'lm-building',4,T.sRGBEncoding,'image/png'),texture(P+'lm-machine.png',T.sRGBEncoding),response(P+'r3-environment.bin','bin'),response(P+'r3-lighting.json','json'),
