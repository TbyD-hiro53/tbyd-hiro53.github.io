...
(function () {
  ...
  function lockScene(lock) {
    document.body.classList.toggle('reading',lock);if(lock){liquidMenu.open(false);liquid.lock(document.getElementById('panel'));}else liquid.unlock();
  ...
  function openPanel(key) {
    ...
    p.inert=false;p.style.visibility='visible';p.setAttribute('aria-hidden','false');p.classList.add('on');liquid.ui.open(p,lastPointer||document.getElementById('h53-menu-toggle'));lastPointer=null;
  ...
  function closePanel() {
    ...
    liquid.ui.close(p,function(){p.classList.remove('on');p.inert=true;p.style.visibility='';p.setAttribute('aria-hidden','true');closeAnimationDone=true;settleClose();});
  }
  ...
  function bind(cv) {
    ...
    cv.addEventListener('pointerup',function(e){
      ...
      if(select){var k=pick(e.clientX,e.clientY);if(k){lastPointer={x:e.clientX,y:e.clientY};openPanel(k);}}
    });
