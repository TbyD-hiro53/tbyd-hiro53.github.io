/* H53 Liquid Glass native THREE r128 / IIFE adapter, derived from the approved
 * Coastal Glass observation v3 optical material without changing its formulas.
 * scene-linear-agx (default when LUT provided): exact approved Coastal pipeline.
 * display-srgb: already tone-mapped display bytes, linear scatter + sRGB encoding.
 * Source texture must exclude the UI. Do not copy the UI-bearing framebuffer.
 */
(function (root) {
  'use strict';
  const T = root.THREE;
  if (!T) throw new Error('H53LiquidGlassMaterial requires THREE r128.');
...
vec3 sceneAt(vec2 uv){vec3 c=clean3(texture2D(uScene,safeUV(uv)).rgb);return uSourceMode<0.5?srgbDecode(c):c;}
...
`;
  ...
  class H53LiquidGlassMaterial {
    constructor(renderer, agxTexture, sourceMode) {
      if (!renderer) throw new Error('The artwork renderer is required.');
      this.sourceMode=sourceMode||(agxTexture?'scene-linear-agx':'display-srgb');
      if(this.sourceMode==='scene-linear-agx'&&!agxTexture)throw new Error('scene-linear-agx requires the artwork LUT.');
      ...
      this.uniforms={
        ...
        uRadius:{value:20},uPanel:{value:0},uOpacity:{value:1},uTransition:{value:1},uSuppression:{value:0},
        uSourceMode:{value:this.sourceMode==='scene-linear-agx'?1:0},uExposure:{value:0},uNight:{value:0},uSimplified:{value:0}
      };
    ...
    draw(sourceLinearTexture, exposure, cssWidth, cssHeight, renderWidth, renderHeight, surfaces, night, simplified) {
      ...
      try{
        ...
        for(let i=0;i<surfaces.length;i++){
          ...
          u.uPanel.value=Number.isFinite(s.panelMix)?unit(s.panelMix,panel?1:0):(panel?1:0);u.uOpacity.value=opacity;u.uTransition.value=unit(s.transition,1);u.uSuppression.value=Math.min(.8,unit(s.suppression,0));
  ...
  root.H53LiquidGlassMaterial=H53LiquidGlassMaterial;
})(globalThis);
