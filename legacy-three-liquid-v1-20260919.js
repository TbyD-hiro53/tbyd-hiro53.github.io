...
function boot(){
 ...
 function message(label,text){view='information';mirrors=[];heading.textContent=label;content.replaceChildren();content.appendChild(el('p',{},text));row('メニューへ戻る',menu);}
 function menu(){view='menu';mirrors=[];heading.textContent='メニュー';content.replaceChildren();
  for(var id of spec.buttons){var original=document.getElementById(id);if(!original)continue;(function(control){var b=row(control.textContent,function(){var locked=host.blocked,until=host.blockedUntil;try{host.blocked=null;host.blockedUntil=0;control.dispatchEvent(new MouseEvent('click',{bubbles:false,cancelable:true}));}finally{host.blocked=locked;host.blockedUntil=until;}sync();},control.id);mirrors.push({source:control,button:b});})(original);}
  ...
  row('UIを隠す',function(){closePanel(function(){hidden=true;top.hidden=true;title.hidden=true;show.hidden=false;body.classList.add('legacy-three-ui-hidden');show.focus({preventScroll:true});});});sync();
 }
 ...
 close.addEventListener('click',function(e){e.stopPropagation();closePanel();});show.addEventListener('click',function(){hidden=false;top.hidden=false;title.hidden=false;show.hidden=true;body.classList.remove('legacy-three-ui-hidden');toggle.focus({preventScroll:true});});
 ...
 active={host:host,canvas:canvas,report:function(){return {slug:slug,view:view,hidden:hidden,state:lastState,buttons:mirrors.map(function(x){return {id:x.source.id,label:x.button.textContent,pressed:x.button.getAttribute('aria-pressed')};}),host:host.report()};},afterRender:function(renderer,state){lastState=state;if(!panel.hidden)sync();host.afterRender(performance.now());}};
